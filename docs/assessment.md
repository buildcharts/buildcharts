...
1) Support version selection: pinned versions for stable, controlled updates, or floating tags like `latest` for faster updates (higher risk), aligned with migrations and deprecations.
...
- No schema needed since the execution runs in Dockerfiles. Use docker syntax or bring your own scripts.
...
- Is it a good idea?
- Compare helm for deploy vs buildcharts for builds (innovation perspective).
