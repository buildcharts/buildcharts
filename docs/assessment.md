...
- **Governance:** Storing charts in a dedicated Git repository enables governence through standard branch protection, mandatory PR reviews, and required automated quality gates. These gates should include Dockerfile linting, static analysis, and policy checks to enforce security, correctness, and versioning rules before charts are published as OCI artifacts. Continuous delivery is used to publish the resulting OCI images to a preferred container registry.
