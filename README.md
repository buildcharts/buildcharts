...
- `buildcharts generate` outputs a Docker bake file `buildcharts.hcl`
  ...
  - Each chart dockerfile will be inlined in the HCL.
- `docker buildx bake` executes the build.
  - Utilizing docker we have isolated, reproducible builds.
  - Built-in support for caching, scaling, metadata, provenance etc.
...
- [docs/build.yaml.md](docs/build.yaml.md) – Metadata specification
