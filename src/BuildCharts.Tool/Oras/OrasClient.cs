...
{
    public static async Task Pull(string reference)
    {
        var tag = "latest";
    ...
    public static async Task Pull(string registry, string repository, string tag, string outputDir = ".buildcharts")
    {
        ...
        // Pulled: docker.io/buildcharts/dotnet-build:0.0.1
        // Digest: sha256:3319c794c58c27ba5a491c51bf064002350391f2fabee587ea8ceccff23fa1f4

        //Console.WriteLine($"Pulling {repository}:{tag}");

        var client = new Client
        {
            CredentialProvider = new SingleRegistryCredentialProvider(registry, new Credential
            {
                Username = "buildcharts",
                Password = "dckr_pat_M8-vjiZTtBTbgepjjVQlrmVYHj8",
            }),
        };
